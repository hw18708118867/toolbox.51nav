    ...
    <TabView :tabs="['加密', '解密']">
      <template #tab-0>
        <div class="space-y-3">
          ...
          <div>
            <label class="text-xs font-medium mb-1 block" style="color: var(--color-text-secondary);">密钥 (Key, 4-56 字节)</label>
...
// 64-bit block, variable key length (32-448 bits), 16-round Feistel network

// Initial P-array and S-boxes from hexadecimal digits of pi
const ORIG_P: number[] = [
...
function blowfishEncryptBlock(L: number, R: number): [number, number] {
  for (let i = 0; i < 16; i++) {
    L ^= P[i];
    R ^= F(L);
    [L, R] = [R, L];
...
function blowfishDecryptBlock(L: number, R: number): [number, number] {
  for (let i = 17; i > 1; i--) {
    L ^= P[i];
    R ^= F(L);
    [L, R] = [R, L];
...
function keySchedule(keyBytes: Uint8Array): void {
  initState();
  ...
  for (let i = 0; i < 18; i++) {
    let data = 0;
    for (let k = 0; k < 4; k++) {
      data = (data << 8) | keyBytes[j % keyLen];
      j++;
    }
    P[i] ^= data;
  }

  // Encrypt zero block to replace P and S
  let L = 0, R = 0;
  for (let i = 0; i < 18; i += 2) {
    [L, R] = blowfishEncryptBlock(L, R);
    P[i] = L;
    P[i + 1] = R;
  }
...
function encryptECB(plaintext: Uint8Array): Uint8Array {
  const paddedLen = plaintext.length + (8 - (plaintext.length % 8)) % 8;
  const result = new Uint8Array(paddedLen);
  for (let i = 0; i < paddedLen; i += 8) {
    let L = 0, R = 0;
    for (let j = 0; j < 4; j++) {
      L = (L << 8) | (i + j < plaintext.length ? plaintext[i + j] : 0);
      R = (R << 8) | (i + j + 4 < plaintext.length ? plaintext[i + j + 4] : 0);
    }
    const [eL, eR] = blowfishEncryptBlock(L, R);
    for (let j = 0; j < 4; j++) {
      result[i + j] = (eL >>> (24 - j * 8)) & 0xff;
      result[i + j + 4] = (eR >>> (24 - j * 8)) & 0xff;
    }
  }
  return result;
}

function decryptECB(ciphertext: Uint8Array): Uint8Array {
  const result = new Uint8Array(ciphertext.length);
  for (let i = 0; i < ciphertext.length; i += 8) {
    let L = 0, R = 0;
    for (let j = 0; j < 4; j++) {
      L = (L << 8) | ciphertext[i + j];
      R = (R << 8) | ciphertext[i + j + 4];
    }
    const [dL, dR] = blowfishDecryptBlock(L, R);
    for (let j = 0; j < 4; j++) {
      result[i + j] = (dL >>> (24 - j * 8)) & 0xff;
      result[i + j + 4] = (dR >>> (24 - j * 8)) & 0xff;
    }
  }
  return result;
}
...
function uint8ArrayToWordArray(u8: Uint8Array, sigBytes?: number): CryptoJS.lib.WordArray {
  const len = sigBytes ?? u8.length;
  const words: number[] = [];
...
function encrypt() {
  ...
  try {
    ...
    const keyStr = key.value || 'DefaultKey';
    const keyBytes = bytesToUint8Array(CryptoJS.enc.Utf8.parse(keyStr));
    keySchedule(keyBytes);

    const plainBytes = bytesToUint8Array(CryptoJS.enc.Utf8.parse(encryptInput.value));
    ...
    if (mode.value === 'ECB') {
      ...
      const paddedLen = plainBytes.length + (8 - (plainBytes.length % 8)) % 8;
      cipherBytes = new Uint8Array(paddedLen);
      let prevL = 0, prevR = 0;
      for (let i = 0; i < paddedLen; i += 8) {
        let L = 0, R = 0;
        for (let j = 0; j < 4; j++) {
          L = (L << 8) | (i + j < plainBytes.length ? plainBytes[i + j] : 0);
          R = (R << 8) | (i + j + 4 < plainBytes.length ? plainBytes[i + j + 4] : 0);
        }
        L ^= prevL; R ^= prevR;
        const [eL, eR] = blowfishEncryptBlock(L, R);
        prevL = eL; prevR = eR;
        for (let j = 0; j < 4; j++) {
          cipherBytes[i + j] = (eL >>> (24 - j * 8)) & 0xff;
          cipherBytes[i + j + 4] = (eR >>> (24 - j * 8)) & 0xff;
        }
      }
    }

    encryptOutput.value = outputFormat.value === 'Base64'
      ? CryptoJS.enc.Base64.stringify(uint8ArrayToWordArray(cipherBytes))
      : CryptoJS.enc.Hex.stringify(uint8ArrayToWordArray(cipherBytes));
  } catch (e: any) {
...
function decrypt() {
  ...
  try {
    ...
    const keyStr = decryptKey.value || 'DefaultKey';
    const keyBytes = bytesToUint8Array(CryptoJS.enc.Utf8.parse(keyStr));
    keySchedule(keyBytes);

    let cipherWords: CryptoJS.lib.WordArray;
    try {
      cipherWords = inputFormat.value === 'Base64'
    ...
    const cipherBytes = bytesToUint8Array(cipherWords);

    let plainBytes: Uint8Array;
    if (decryptMode.value === 'ECB') {
      plainBytes = decryptECB(cipherBytes);
    } else {
      // CBC mode
      plainBytes = new Uint8Array(cipherBytes.length);
      let prevL = 0, prevR = 0;
      for (let i = 0; i < cipherBytes.length; i += 8) {
        let L = 0, R = 0;
        for (let j = 0; j < 4; j++) {
          L = (L << 8) | cipherBytes[i + j];
          R = (R << 8) | cipherBytes[i + j + 4];
        }
        const currentL = L, currentR = R;
        const [dL, dR] = blowfishDecryptBlock(L, R);
        const pL = dL ^ prevL, pR = dR ^ prevR;
        prevL = currentL; prevR = currentR;
        for (let j = 0; j < 4; j++) {
          plainBytes[i + j] = (pL >>> (24 - j * 8)) & 0xff;
          plainBytes[i + j + 4] = (pR >>> (24 - j * 8)) & 0xff;
        }
      }
    }

    decryptOutput.value = CryptoJS.enc.Utf8.stringify(uint8ArrayToWordArray(plainBytes));
    if (!decryptOutput.value && cipherBytes.length > 0) {
