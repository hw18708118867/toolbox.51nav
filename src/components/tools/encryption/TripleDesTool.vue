    ...
    <TabView :tabs="['加密', '解密']">
      <template #tab-0>
        <div class="space-y-3">
          ...
          <div>
            <label class="text-xs font-medium mb-1 block" style="color: var(--color-text-secondary);">密钥 (24 字符)</label>
            <input v-model="key" type="text" placeholder="请输入 24 字符密钥" class="w-full rounded-md border px-3 py-2 text-sm focus:outline-none "  />
          </div>
...
function encrypt() {
  ...
  try {
    ...
    const encrypted = CryptoJS.TripleDES.encrypt(encryptInput.value, keyHex, {
...
function decrypt() {
  ...
  try {
    ...
    const decrypted = CryptoJS.TripleDES.decrypt(decryptInput.value, keyHex, {
      mode: decryptMode.value === 'ECB' ? CryptoJS.mode.ECB : CryptoJS.mode.CBC,
      padding: CryptoJS.pad.Pkcs7,
    });
